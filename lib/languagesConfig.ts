export const languagesConfig = {
  kirundi: {
    name: "Kirundi",
    image: "/images/kirundi.png",
    books: [
      {
        title: "Kirundi Book 1",
        slug: "kirundi-book-1",
        description: "Description of Kirundi Book 1",
        cover: "/images/kirundi-book-1.png",
        challenges: ["Challenge 1", "Challenge 2", "Challenge 3"],
        whyChoose: ["Reason 1", "Reason 2", "Reason 3"],
        testimonials: [
          "This book is amazing! - Parent A",
          "My kids love it! - Parent B",
          "Highly recommended! - Parent C",
        ],
        faq: [
          {
            question: "What is the purpose of Kirundi books?",
            answer: "To preserve and promote the Kirundi language and culture.",
          },
        ],
      },
      {
        title: "Kirundi Book 2",
        slug: "kirundi-book-2",
        description: "Description of Kirundi Book 2",
        challenges: [],
        whyChoose: [],
        testimonials: [],
      },
    ],
    bestSellers: [
      {
        title: "Kirundi Book 1",
        slug: "kirundi-book-1",
        description: "Description of Kirundi Book 1",
        cover: "/images/kirundi-book-1.png",
      },
      {
        title: "Kirundi Book 2",
        slug: "kirundi-book-2",
        description: "Description of Kirundi Book 2",
        cover: "/images/kirundi-book-2.png",
      },
      {
        title: "Kirundi Book 3",
        slug: "kirundi-book-3",
        description: "Description of Kirundi Book 3",
        cover: "/images/kirundi-book-3.png",
      },
    ],
    faq: [
      {
        question: "What is the purpose of Kirundi books?",
        answer: "To preserve and promote the Kirundi language and culture.",
      },
    ],
  },
  kinyarwanda: {
    name: "Kinyarwanda",
    image: "/images/kinyarwanda.png",
    books: [
      {
        title: "Kinyarwanda Book 1",
        slug: "kinyarwanda-book-1",
        description: "Description of Kinyarwanda Book 1",
        challenges: [],
        whyChoose: [],
        testimonials: [],
      },
      {
        title: "Kinyarwanda Book 2",
        slug: "kinyarwanda-book-2",
        description: "Description of Kinyarwanda Book 2",
        challenges: [],
        whyChoose: [],
        testimonials: [],
      },
    ],
    bestSellers: [
      {
        title: "Kinyarwanda Book 1",
        slug: "kinyarwanda-book-1",
        description: "Description of Kinyarwanda Book 1",
        cover: "/images/kinyarwanda-book-1.png",
      },
      {
        title: "Kinyarwanda Book 2",
        slug: "kinyarwanda-book-2",
        description: "Description of Kinyarwanda Book 2",
        cover: "/images/kinyarwanda-book-2.png",
      },
      {
        title: "Kinyarwanda Book 3",
        slug: "kinyarwanda-book-3",
        description: "Description of Kinyarwanda Book 3",
        cover: "/images/kinyarwanda-book-3.png",
      },
    ],
    faq: [
      {
        question: "What is the purpose of Kinyarwanda books?",
        answer: "To preserve and promote the Kinyarwanda language and culture.",
      },
    ],
  },
  lingala: {
    name: "Lingala",
    image: "/images/lingala.png",
    books: [
      {
        title: "Lingala Book 1",
        slug: "lingala-book-1",
        description: "Description of Lingala Book 1",
        challenges: [],
        whyChoose: [],
        testimonials: [],
      },
      {
        title: "Lingala Book 2",
        slug: "lingala-book-2",
        description: "Description of Lingala Book 2",
        challenges: [],
        whyChoose: [],
        testimonials: [],
      },
    ],
    faq: [
      {
        question: "What is the purpose of Lingala books?",
        answer: "To preserve and promote the Lingala language and culture.",
      },
    ],
  },
};
